## ---------- Script -----------------
##
## Purpose: perform differential analysis on mouse rTg4510 Iso-Seq targeted datasets using linear regression
##
...
dirnames <- list(
  ...
  output = "/gpfs/mrc0/projects/Research_Project-MRC148213/sl693/scripts/rTg4510/B_Targeted_Transcriptome/"
)

input_files <- list(
  phenotype = paste0(dirnames$rTg4510, "0_metadata/B_isoseq_targeted/TargetedMouse_PhenotypeTAPPAS.txt"), 
...
input$phenotype$sample <- str_replace(input$phenotype$sample,"FL.","isoseq_")
...
write.csv(res_df, paste0(dirnames$output, "IsoSeq_DESeq2_results.csv"))

# apply and output
pdf(paste0(dirnames$output,"Sig0.1_IsoSeq_DESeq2_TargetedTranscriptome.pdf"), width = 8, height = 5)
