# Szi Kay Leung
# Function for tappAS related plots
...
### tappAS Input files #################################################################
input_tappasfiles <- function(tappas_input_dir, type){
  ...
  tappasfiles <- list(paste0(tappas_input_dir,"/Data/gene_matrix.tsv"),
                      ...
                      paste0(tappas_input_dir,"/InputData/exp_factors.txt"),
                      paste0(tappas_input_dir,"/Data/result_matrix.tsv"))
  
  tappasfiles <- lapply(tappasfiles, function(x) read.table(x, sep = "\t", header = T))
  names(tappasfiles) <- c("gene_matrix.tsv","gene_transcripts.tsv","input_norm","result_gene_trans.tsv","transcript_matrix.tsv","exp_factors.txt","result_matrix.tsv")
  ...
  if(type == "rnaseq"){
    ...
    for(c in 1:length(colnames(tappasfiles$input_norm))){
      colnames(tappasfiles$input_norm)[c] <- 
        tappasrna_phenotype[colnames(tappasfiles$input_norm)[c] == tappasrna_phenotype$sample,"variable"]
    }
  }
...
tappas_resultsanno <- function(classification_file, tappas_normalised_expmatrix,phenotype){
  ...
  ### Annotate tappas data with the phenotype and classification file, prequisite for plots
...
tabulate_all_diffgenes <- function(){
  Allsig_output = list()
  # Interaction genes only (365 genes)
  Allsig_output = lapply(Interaction$...1, function(x) tabulate_diffgene(x, IsoExp$GeneExp, Interaction))
