...
###########################################################################################
# Merged Data
Merged_Data_Input <- function(){
  ## Define directory input
  sqanti_dir <- "/gpfs/mrc0/projects/Research_Project-MRC148213/sl693/WholeTranscriptome/WT8_ISOSEQ/IsoSeq3.1.2/SQANTI2"
  tofu_dir <- "/gpfs/mrc0/projects/Research_Project-MRC148213/sl693/WholeTranscriptome/WT8_ISOSEQ/IsoSeq3.1.2/TOFU"
  featurecount_dir <- "/gpfs/mrc0/projects/Research_Project-MRC148213/sl693/WholeTranscriptome/WT8_ISOSEQ/Post/FeatureCounts"
  
  classification_file <- list.files(path = sqanti_dir , pattern = ".collapsed.filtered.rep_classification.filtered_lite_classification.txt", full.names = TRUE)
  classification_dat <- read.delim2(file = classification_file, header = TRUE)
  classification_dat <<- classification_dat
  
  abundance_file <- list.files(path = tofu_dir , pattern = ".collapsed.filtered.abundance.txt", full.names = TRUE)
  ...
  featurecount_file <- list.files(path = featurecount_dir, pattern = "WT8_Merged_gene_genenames.mx", full.names = TRUE) 
  print(featurecount_file)
  featurecount_dat <- read.delim2(featurecount_file, row.names = 1, header = TRUE)
  
  # save featurecount_dat merged as RNASeq for downstream analysis labelling
  RNASeq <- featurecount_dat
...
Prepare_Gencode <- function(gencode){
  ...
  print("Gencode vM20 Gene Annotation with summed transcripts per gene")
  datatable(gencode_count)
}
