...
HKGeneExpPlots <- list()
PbGeneId = unique(word(subset(class.files$glob_iso, associated_gene %in% housekeepingGenes)[["isoform"]],c(2),sep=fixed(".")))
for(i in PbGeneId){
  HKGeneExpPlots[[i]] <- plot_trans_exp_individual_overtime(i,GlobalDESeq$resGeneAnno$wald$norm_counts,type="gene")
}
names(HKGeneExpPlots) <- housekeepingGenes 

# most abundant top-ranked expression plot
HKTransExpPlots <- list()
for(i in housekeepingGenes){
  HKTransExpPlots [[i]] <- plot_transexp_overtime(i,GlobalDESeq$resTranAnno$wald$norm_counts,show="toprank",rank=3,setorder=c("CONTROL","CASE"))
}

plot_grid(plotlist = HKGeneExpPlots)
plot_grid(plotlist = HKTransExpPlots)
