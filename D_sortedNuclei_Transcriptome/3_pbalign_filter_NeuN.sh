...
#SBATCH --output=Output/3log/3_pbalign_filter_NeuN-%A_%a.o
#SBATCH --error=Output/3log/3_pbalign_filter_NeuN-%A_%a.e
...
NeuNfa=($(find "${WKD_ROOT}/4_tclean/NeuN" -type f -name "*clean.fa")) 
fasta=${NeuNfa[${SLURM_ARRAY_TASK_ID}]}
...
echo "Output: ${WKD_ROOT}/5_cupcake/5_align/${sample}_NeuN_mapped.bam"
source activate isoseq3
cd ${WKD_ROOT}/5_cupcake/5_align
pbmm2 align --preset ISOSEQ --sort ${GENOME_FASTA} ${fasta} ${sample}_NeuN_mapped.bam --log-level TRACE --log-file ${sample}_mapped.log

# filter_alignment <input_name> <input_mapped_dir>
# output = ${sample}_mapped.filtered.bam, ${sample}_mapped.filtered.sorted.bam
filter_alignment ${sample}_NeuN_mapped ${WKD_ROOT}/5_cupcake/5_align