...
input_dir <- "/gpfs/mrc0/projects/Research_Project-MRC148213/sl693/IsoSeq/Whole_Transcriptome/All_Merged/DEMUX_CUSTOM_ADJUST/SQANTI_TAMA_FILTER/"
# read in SQANTI2 classification file of all merged data
class <- read.table(paste0(input_dir,"WholeIsoSeq.collapsed_sqantitamafiltered.classification.txt"),header=T)
...
countdata <- class[,c("transcript_name_id",paste("FL",samples, sep = "."))] %>% column_to_rownames(., var = "transcript_name_id")
countdata <- as.matrix(countdata)
(condition <- factor(c(rep("WT", 6), rep("TG", 6))))
(age <- factor(c(rep("WT", 6), rep("TG", 6))))
...
write.csv(as.data.frame(resdata), file="DESeq2_IsoseqFL_DemuxTama_TranscriptExpression.csv")
