...
input_dir <- "/gpfs/mrc0/projects/Research_Project-MRC148213/sl693/IsoSeq/Whole_Transcriptome/All_Samples/MOUSE/DEMUX_WT/"

##### 1. Input #####
##### Input SQANTI2 classification file, and prepare counts and abundance

# Read in SQANTI2 classification file of all merged data
class <- read.table(paste0(input_dir,"WT_Merged.collapsed_sqantitamafiltered.classification.txt"),header=T)

# Prepare Counts: keep only PacBio isoform ID and columns with isoform FL counts 
samples <- c("K17_WT_2mos","M21_WT_2mos","Q21_WT_2mos","S23_WT_8mos","O23_WT_8mos","K23_WT_8mos")
...
input_abundance <- read.table(paste0(input_dir,"WT.Demultipled_Abundance.txt"),header=T, sep = ",")
...
abundance[2:7] <- lapply(abundance[2:7],function(x) as.numeric(as.character(x)))

# check abundance correctly determined
abundance[abundance$isoform_id == "PB.11869.1",c("FL.K17_WT_2mos")]
counts[,c("isoform_id","FL.K17_WT_2mos")] %>% mutate(K17_abundance = FL.K17_WT_2mos / total$K17_WT_2mos * 1000000) %>% filter(isoform_id == "PB.11869.1")
...
myDesign <- data.frame(
  ...
  condition = c(rep("WT_2mos",3),rep("WT_8mos",3))
)
...
aSwitchList <- importRdata(
  ...
  isoformExonAnnoation = paste0(input_dir,"WT_Merged.collapsed_sqantitamafiltered.classification.gtf"),
  isoformNtFasta       = paste0(input_dir,"WT_Merged.collapsed_sqantifiltered_tamafiltered_classification.fasta"),
)
