...
#SBATCH --output=Diff_Targeted2b.o
#SBATCH --error=Diff_Targeted2b.e
...
#cd $DiffAnalysis_WKD; mkdir SQANTI3 SQANTI_TAMA_FILTER RNASeq_SQANTI3 TAPPAS_INPUT COLLAPSE_FILTER
#cd $DiffAnalysis_WKD/RNASeq_SQANTI3; mkdir Individual Individual_Collapsed
...
#run_sqanti3 $dataset".collapsed" $dataset".collapsed.gff" $PostIsoseq3_WKD/TOFU $RNASeq_WKD/MAPPED $PostIsoseq3_WKD/KALLISTO/AllMouseTargeted.mod.abundance.tsv $PostIsoseq3_WKD/TOFU/AllMouseTargeted.Demultiplexed_Abundance.txt $DiffAnalysis_WKD/SQANTI3 genome
...
#Rscript $DIFF_FUNC/Sqanti_Collapsed_Counts.R $DiffAnalysis_WKD/SQANTI3/$sqname"_classification.txt" $DiffAnalysis_WKD/SQANTI_TAMA_FILTER/$dataset"_sqantitamafiltered.classification.txt" $DiffAnalysis_WKD/TAPPAS_INPUT/IsoSeq_Expression/AllMouseTargeted_sqantitamafiltered_collapsed.expression.txt $DiffAnalysis_WKD/TAPPAS_INPUT/tama_retained_collapsed_pbid.txt Targeted

## 12) TAMA_sqanti_filter <TAMA_remove_fragments.output> <sqanti_filtered_dir> <sqanti_output_txt> <sqanti_output_gtf> <sqanti_output_fasta> <output_prefix_name> <output_dir>
# rerun on command line though logged files
sqname=$dataset".collapsed_classification.filtered_lite"
collapse_filter $DiffAnalysis_WKD/TAPPAS_INPUT/tama_retained_collapsed_pbid.txt $DiffAnalysis_WKD/SQANTI3 $sqname"_classification.txt" $sqname".gtf" $sqname".fasta" $sqname"_junctions.txt" $dataset $DiffAnalysis_WKD/COLLAPSE_FILTER
...
# rerun on command line though logged files
cd $DiffAnalysis_WKD/RNASeq_SQANTI3/Individual_Collapsed
