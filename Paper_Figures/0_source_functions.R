...
plot_boxplot_SCN <- function(normCounts, iso, ageDiv = TRUE){
  
  dat <- normCounts %>% filter(isoform %in% iso) %>% mutate(genotype = factor(genotype, levels = c("WT","TG"))) 
...
BDR_plot <- function(norm_counts, iso = NULL, gene = NULL, sampleExclude = NULL, IF = NULL, Braak = FALSE){
  ...
  if(isFALSE(Braak)){
    dat <- dat %>%  filter(BraakTangle_numeric %in% c(0,1,2,5,6))  %>%
      mutate(phenotype = ifelse(BraakTangle_numeric %in% c(0,1,2),"Control","AD")) %>%
      mutate(phenotype = factor(phenotype, levels = c("Control","AD")))
    p <- ggplot(dat, aes(x = phenotype, y = normalised_counts)) + geom_boxplot() + theme_classic() 
  }else{
    p <- ggplot(dat, aes(x = BraakTangle_numeric, y = normalised_counts)) + geom_boxplot() + theme_classic() 
  }
...
plot_protein_general <- function(pclassfile){
  
  RNATranscript <- pclassfile %>% group_by(associated_gene) %>% dplyr::summarize(RNATranscript = n())
  ...
  UniqueORF <- pclassfile %>% filter(!is.na(corrected_acc)) %>% 
  ...
  p1 <- ggplot(NumTranscriptIsoform, aes(x = RNAIsoform, y = RNATranscript, label = associated_gene)) + 
    ...
    labs(x = "Number of RNA isoforms with unique CDS", y = "Number of RNA transcripts")
  ...
  p2 <- ggplot(UniqueORF, aes(y = n, x = subcategory)) + geom_bar(stat = "identity") + 
    ...
    coord_flip() + 
    labs(y = "Number of RNA Isoforms with unique CDS", x = "Subcategory") + 
...
visualise_ORFs <- function(refgtf,tgtf, pgtf, tclassfiles, pclassfiles, gene, transcript = NULL, cpat = NULL, species = NULL){
  ...
  if(!is.null(transcript)){
    ...
    pID <- pclassfiles %>% filter(corrected_acc == transcript) %>% .[["isoform"]]
    datIDs <- list(
      Reference = refIDs,
      `RNA Transcript` = pclassfiles %>% filter(corrected_acc == transcript) %>% .[["isoform"]],
      `RNA Isoform` = unique(pgtf %>% dplyr::filter(transcript %in% pID) %>% .[["gene_id"]])
    )
    ...
    proteinIDs <- list(Reference = refIDs,`RNA Isoform` = tORFID) 
    
    pT <- generalggTranPlots(transcriptIDs, tgtf, tclassfiles, gene)
    pP <- generalggTranPlots(proteinIDs, tgtf, tclassfiles, gene, cpat, species)
